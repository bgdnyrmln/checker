    ...
    <main class="flex-1 pl-[22rem] min-h-screen relative overflow-hidden">
      ...
      <div class="relative z-10 max-w-[96rem] mx-auto py-[4rem] px-[4rem]">
        ...
        <div class="mb-[4rem]">
          ...
          <h1 class="page-title text-[3.6rem] leading-none tracking-wide">My Sick Leaves</h1>
        </div>

        <!-- Submit form -->
        <div class="filter-card rounded-[1.6rem] p-[2.8rem] mb-[2.4rem]">
          <p class="page-label text-[1rem] tracking-[0.18em] uppercase mb-[2rem]">New Record</p>

          <div class="flex flex-wrap items-end gap-[1.6rem]">

            <!-- Dates -->
            <div class="flex items-end gap-[1.2rem]">
              <div class="flex flex-col gap-[0.4rem]">
                <label class="page-label text-[1rem] tracking-[0.15em] uppercase">Start Date</label>
                <input type="date" v-model="form.date_start" class="date-input rounded-[0.8rem] px-[1.4rem] py-[0.9rem] text-[1.3rem] outline-none transition-all duration-200" />
              </div>
              <div class="arrow-sep text-[1.4rem] mb-[0.9rem]">→</div>
              <div class="flex flex-col gap-[0.4rem]">
                <label class="page-label text-[1rem] tracking-[0.15em] uppercase">End Date</label>
                <input type="date" v-model="form.date_end" class="date-input rounded-[0.8rem] px-[1.4rem] py-[0.9rem] text-[1.3rem] outline-none transition-all duration-200" />
              </div>
            </div>
            ...
            <div class="flex flex-col gap-[0.4rem]">
              ...
              <label class="file-label flex items-center gap-[1rem] rounded-[0.8rem] px-[1.4rem] py-[0.9rem] cursor-pointer transition-all duration-200">
                ...
                <span class="text-[1.2rem]" :class="selectedFile ? 'file-name-active' : 'page-label'">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-[3rem] py-[1.6rem] text-[1.3rem]">
                  <span v-if="s.date_end" class="table-sub tabular-nums">
                ...
                <td class="px-[3rem] py-[1.6rem]">
                  <button
                    v-if="s.has_file"
                    @click="viewFile(s.id)"
                    class="view-btn flex items-center gap-[0.6rem] px-[1.2rem] py-[0.5rem] rounded-[0.6rem] text-[1.1rem] transition-all duration-200"
                  >
                    <svg class="w-[1.2rem] h-[1.2rem]" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M18.375 12.739l-7.693 7.693a4.5 4.5 0 01-6.364-6.364l10.94-10.94A3 3 0 1119.5 7.372L8.552 18.32m.009-.01l-.01.01m5.699-9.941l-7.81 7.81a1.5 1.5 0 002.112 2.13"/>
                    </svg>
